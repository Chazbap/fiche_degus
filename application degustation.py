...
# Demander à l'utilisateur de télécharger les fichiers Excel
st.subheader("1️⃣ Téléchargez vos fichiers Excel")

uploaded_file_cuves = st.file_uploader("Téléchargez le fichier des cuves (Excel)", type=["xlsx"])
uploaded_file_codes = st.file_uploader("Téléchargez le fichier des codes produits (Excel)", type=["xlsx"])

if uploaded_file_cuves is not None and uploaded_file_codes is not None:
    ...
    st.write("Aperçu des cuves :", df_cuves.head())
    st.write("Aperçu des codes produits :", df_codes.head())

else:
    st.warning("Veuillez télécharger les deux fichiers Excel pour continuer.")
    st.stop()

# Demander le nombre de dégustateurs
st.subheader("2️⃣ Nombre de dégustateurs")
...
# Sélection des cuves
st.subheader("3️⃣ Sélection des cuves")
...
st.subheader("4️⃣ Saisie des notes et affichage des moyennes")
